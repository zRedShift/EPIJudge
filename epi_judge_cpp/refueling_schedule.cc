...
int FindAmpleCity(const vector<int> &gallons, const vector<int> &distances) {
  int i = 0, j = distances.size() - 1, distance = distances[j], mileage = gallons[j] * kMPG;
  while (i != j)
    if (distance > mileage)
      distance += distances[--j], mileage += gallons[j] * kMPG;
    else
      distance += distances[i], mileage += gallons[i++] * kMPG;
  return i;
}
