...
class ServiceRequest {
 public:
  explicit ServiceRequest(string s) : request_(move(s)) {}
...
class SpellCheckService {
 public:
  static void Service(ServiceRequest &req, ServiceResponse &resp) {
    ...
    lock.unlock();
    auto response = ClosestInDictionary(w);
    resp.EncodeIntoResponse(response);
    lock.lock();
    w_last_ = move(w);
    closest_to_last_word_ = move(response);
  }
...
mutex SpellCheckService::mutex_;
